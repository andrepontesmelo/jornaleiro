...
module Jornaleiro
  ...
  RSpec.describe ".MetaDocuments" do
    ...
    it "should find an already inserted session" do
      ...
      expect(m.has_session?(55)).to be_truthy
    end
    ...
    it "should retrieve a document by session id " do
      ...
      expect(m.get_document(55)).to eq(documento)
    end

    it "should get the list of documents as a array" do
      expect(m.get_documents.class).to be(Array.new.class)
    end
    ...
    it "should retrieve the list of documents inserted" do
      ...
      m.push(document)
      expect(m.get_documents[0]).to eq(document)
    end

    it "should'nt allow adding more then one document in the same session" do
    ...
    it "should merge a list of pdfs of all sessions" do
      ...
      expect(m.merge_links).to eq(['pdf1', 'pdf2', 'pdf3', 'pdf4'])
    end
  end
end
